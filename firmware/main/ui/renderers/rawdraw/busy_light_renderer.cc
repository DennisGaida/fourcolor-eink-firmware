...
int CameraGlyphWidth(bool on, const lv_font_t* font) {
    ...
    const char* label = on ? i18n::Tr("摄像头开启", "CAM ON") : i18n::Tr("摄像头关闭", "CAM OFF");
...
void BusyLightRenderer::RenderDetailFace(uint8_t* fb, int width, int height) {
    ...
    const char* word = busy_now ? i18n::Tr("会议", "MEETING") : i18n::Tr("空闲", "FREE");
    ...
    if (earlier_count > 0) {
        char buf[24];
        snprintf(buf, sizeof(buf), i18n::Tr("↑%d 更早", "%d earlier"), earlier_count);
    ...
    if (later_count > 0) {
        char buf[24];
        snprintf(buf, sizeof(buf), i18n::Tr("↓%d 更晚", "%d later"), later_count);
        const int w = MeasureTextWidth(buf, font_);
...
void BusyLightRenderer::RenderDefaultFace(uint8_t* fb, int width, int height) {
    ...
    if (real_active) {
        char buf[24];
        snprintf(buf, sizeof(buf), "%s %02d:%02d", i18n::Tr("直到", "until"),
        ...
        if (next) {
            char buf[32];
            snprintf(buf, sizeof(buf), "%s %02d:%02d", i18n::Tr("下一项", "until"),
                     static_cast<int>(next->start_minutes / 60), static_cast<int>(next->start_minutes % 60));
            until_line = buf;
        } else {
            until_line = i18n::Tr("今日无日程", "free all day");
        }
    }
    ...
    if (status.is_adhoc) {
        ...
        seg_b = i18n::Tr("临时通话", "Ad-hoc call");
        seg_b_color = RED;
    } else if (!busy_now) {
        seg_b = i18n::Tr("可以进来", "Come on in");
    } else {
        switch (status.tier) {
            case PresenceEventTier::kLeadership:
                seg_a = i18n::Tr("内部 — ", "Internal — ");
                seg_b = i18n::Tr("重要", "important");
                seg_b_color = RED;
                break;
            case PresenceEventTier::kCustomer:
                seg_b = i18n::Tr("客户", "Customer");
                seg_b_color = RED;
                break;
            case PresenceEventTier::kInternal:
            default:
                seg_a = i18n::Tr("内部 — ", "Internal — ");
                seg_b = i18n::Tr("可以进来", "come in");
                break;
        }
    }
    ...
    if (eff_presenting) {
        ...
        const char* banner_text = i18n::Tr("演示中 — 请勿打扰", "Presenting — do not disturb");
    ...
    const char* footer_hint = i18n::Tr("按键查看今日计划", "press for today's plan");
